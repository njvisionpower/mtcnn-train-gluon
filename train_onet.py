...
lr_steps = [30000, 50000, 60000, 70000]
lr_decay = 0.1
save_interval = 5000
max_iter = 80000
ctx = mx.gpu()
...
train_anno_path += [os.path.join(root_dir, "train_faces_o/pos/image_pos")]
train_anno_path += [os.path.join(root_dir, "train_faces_o/pos/label_pos")]

train_anno_path += [os.path.join(root_dir, "train_faces_o/part/image_part")]
train_anno_path += [os.path.join(root_dir, "train_faces_o/part/label_part")]

train_anno_path += [os.path.join(root_dir, "train_faces_o/neg/image_neg")]
train_anno_path += [os.path.join(root_dir, "train_faces_o/neg/label_neg")]
start_epoch = 0
...
for k in range(start_epoch, max_iter + 1):
    ...
    if k % 100 == 0:
        ...
        reg_auc = eval.AddBoxMap(box, targets, 48, 48)
    ...
    if k % 5000 == 0:
        net.save_parameters('./models/onet_' + str(k))
